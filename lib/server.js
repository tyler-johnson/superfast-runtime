...
app.set({
	superfast: { dir: SFDIR },
	browserKeys: ["env","log","name","version","superfast.build.seq"]
});

function updateBuild() {
	let src = fs.readFileSync(join(SFDIR, "package.json"), { encoding: "utf-8" });
	app.reset("superfast.build", JSON.parse(src));
}
...
io.on("connection", function(socket) {
	socket.on("build", function(fn) {
		fn(app.get("superfast.build.seq"));
	});
});

process.on("message", function(msg) {
	if (msg === "SOURCE_UPDATE") {
		updateBuild();
		io.sockets.emit("source_update");
	}
});
