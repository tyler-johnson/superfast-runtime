...
export function helloClient() {
	this.socket.on("connect", () => {
		this.call("serverInfo", this.wait((err, info) => {
			...
			// make sure the client is the same instance as the server
			if (this.env === "development") {
				let selfid = this.get("id");
				if (selfid != null && info && info.id && info.id !== selfid) {
					location.reload();
				}
			}
			...
			if (ver != null && info && info.version != null && info.version !== ver) {
				location.reload();
			}
		}));
	});
